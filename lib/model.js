...
class Model {
  ...
  static async findOrCreate(options) {
    ...
    try {
      ...
      options.transaction = await this.sequelize.transaction(options);
      const instance = await this.findOne(Utils.defaults({ transaction }, options));
      if (instance !== null) {
      ...
      try {
        const createdInstance = await this.create(values, options);
        if (createdInstance.get(this.primaryKeyAttribute, { raw: true }) === null) {
        ...
        const foundInstance = await this.findOne(Utils.defaults({
          transaction: internalTransaction ? null : transaction
        }, options));
        // Sanity check, ideally we caught this at the defaultFeilds/err.fields check
        // But if we didn't and instance is null, we will throw
        if (foundInstance === null) throw err;
  ...
  static async upsert(values, options) {
    ...
    if (options.hooks) {
      await this.runHooks('afterUpsert', res, options);
    }
    return res;
  }
